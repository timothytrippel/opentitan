...
static status_t read_and_print_flash_info_0_data(void) {
  ...
  LOG_INFO("AST Calibration Values (from flash):");
  ...
  for (size_t i = 0; i < kFlashInfoAstCalibrationDataSizeIn32BitWords; ++i) {
    LOG_INFO("Word %2d = 0x%08x", i, ast_cfg_data[i]);
  }
...
static void manually_init_ast(uint32_t *data) {
  // Program the AST configuration CSRs.
  for (size_t i = 0; i < kFlashInfoAstCalibrationDataSizeIn32BitWords; ++i) {
    abs_mmio_write32(TOP_EARLGREY_AST_BASE_ADDR + i * sizeof(uint32_t),
                     data[i]);
  }

  // Readback the AST configuration CSRs.
  LOG_INFO("AST Calibration Values (from AST CSRs):");
  for (size_t i = 0; i < kFlashInfoAstCalibrationDataSizeIn32BitWords; ++i) {
    LOG_INFO(
        "Word %2d = 0x%08x", i,
...
bool test_main(void) {
  ...
  // Enable external clock on silicon platforms.
