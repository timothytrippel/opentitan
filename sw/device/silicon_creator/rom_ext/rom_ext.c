...
static rom_error_t rom_ext_verify(const manifest_t *manifest,
  ...
  sigverify_spx_config_id_t spx_config = kSigverifySpxConfigIdSha2128s;
  rom_error_t rv = sigverify_spx_verify(
  ...
  if (rv == kErrorOk) {
    dbg_printf("%s:%d SPX verified\r\n", __func__, __LINE__);
  } else {
    dbg_printf("%s:%d SPX failed\r\n", __func__, __LINE__);
  }
  return rv;
}
